...
const FormDemo = React.createClass({
  ...
  handleSubmit(e) {
    ...
    validation.validate((valid) => {
      ...
      this.props.add(this.state.formData.name);
    });
  },
          ...
          <FormItem
            ...
            required>
              <Validator rules={[{required: true, min: 5, message: '用户名至少为 5 个字符'}, {validator: this.userExists}]}>
                <Input name="name" id="name" value={formData.name} placeholder="实时校验，输入 zhongxia 看看" onChange={this.setField.bind(this, 'name')} />
              </Validator>
          </FormItem>
